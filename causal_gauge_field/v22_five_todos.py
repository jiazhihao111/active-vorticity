"""
v22_five_todos.py — 五项待办综合实验

待办1: Qwen2.5-7B跨模型A/B实验
待办2: 扩大样本量至20+条prompt
待办3: 区分prefill vs decode阶段rnd数据
待办4: vLLM集成原型(文档+接口设计)
待办5: 仿射子空间压缩在线校准优化
"""
...
def load_model(model_id="minicpm5-1b", quantization="bf16"):
    from transformers import AutoModelForCausalLM, AutoTokenizer, BitsAndBytesConfig

    model_paths = {
        "minicpm5-1b": r"C:\Users\51615\.cache\modelscope\MiniCPM5-1B",
        "qwen2.5-7b": r"C:\Users\51615\.cache\modelscope\Qwen2___5-7B-Instruct",
    }

    path = model_paths[model_id]
    print(f"  Loading {model_id} ({quantization})...")
    ...
    if quantization == "4bit":
        ...
        model = AutoModelForCausalLM.from_pretrained(
            path, quantization_config=qconfig, device_map="auto", trust_remote_code=True,
        )
    else:
        model = AutoModelForCausalLM.from_pretrained(
...
def generate_trajectory(model, tokenizer, prompt, max_new_tokens=25, collect_prefill=False):
    """Generate trajectory with optional prefill hidden states collection."""
    prefill_states = []
    decode_states = []
...
def run_ab_experiment(model, tokenizer, alpha_star, prompts_by_type, model_label, collect_prefill=False):
    """Run full A/B analysis on a model."""
...
def run_affine_compression(model, tokenizer, alpha_star, prompts, model_label):
    """待办5: 仿射子空间压缩在线校准优化."""
    print(f"\n[待办5] Affine compression online calibration ({model_label})...")
    ...
    return {
        ...
        "cross_prompt": True,
        "note": "SVD basis from prompt[0], tested on prompt[1] — cross-prompt basis expected to fail for logits but OK for monitoring",
    }


def main():
    print("=" * 70)
    print("GUIT-TRT 五项待办综合实验")
    ...
    # === 待办1+2+3: MiniCPM5-1B (20 prompts, prefill/decode) ===
    print("\n[待办1+2+3] MiniCPM5-1B bf16 (20 prompts, prefill vs decode)...")
    model, tokenizer = load_model("minicpm5-1b", "bf16")
    ...
    mini_results = run_ab_experiment(model, tokenizer, 1.46, prompts_mini, "MiniCPM5-1B-bf16", collect_prefill=True)
    all_results["minicpm5_1b_bf16_20prompts"] = mini_results

    # === 待办5: MiniCPM5-1B affine compression ===
    mini_affine = run_affine_compression(model, tokenizer, 1.46, POS_PROMPTS_20, "MiniCPM5-1B")
    if mini_affine:
        all_results["minicpm5_1b_affine_compression"] = mini_affine

    del model
    torch.cuda.empty_cache()

    # === 待办1: Qwen2.5-7B bf16 ===
    print("\n[待办1] Qwen2.5-7B bf16 (20 prompts)...")
    try:
        model_q, tokenizer_q = load_model("qwen2.5-7b", "bf16")

        prompts_qwen = {
            "pos": POS_PROMPTS_20[:10],
            "scr": SCR_PROMPTS_20[:10],
            "rnd": RND_PROMPTS_20[:10],
        }

        qwen_results = run_ab_experiment(model_q, tokenizer_q, 1.41, prompts_qwen, "Qwen2.5-7B-bf16", collect_prefill=True)
        all_results["qwen25_7b_bf16_10prompts"] = qwen_results

        # 待办5: Qwen2.5-7B affine compression
        qwen_affine = run_affine_compression(model_q, tokenizer_q, 1.41, POS_PROMPTS_20, "Qwen2.5-7B")
        if qwen_affine:
            all_results["qwen25_7b_affine_compression"] = qwen_affine

        del model_q
        torch.cuda.empty_cache()
    except Exception as e:
        print(f"  Qwen2.5-7B failed: {e}")
        all_results["qwen25_7b_error"] = str(e)

    # === 待办4: vLLM集成设计 ===
    all_results["vllm_integration_design"] = {
        "status": "DESIGN_ONLY",
        "current_blocker": "vLLM requires Linux; Windows not natively supported. WSL2 alternative available.",
        "proposed_interface": {
            "class": "ThermoPagedAttention",
            "methods": ["register_thermo_probe()", "evict_by_causal_score()", "get_thermo_metrics()"],
            "integration_point": "vllm.worker.model_runner.ModelRunner._run_encoder_or_decoder",
            "key_advantage": "PagedAttention block manager enables true VRAM savings (unlike HF manual forward)",
            "estimated_vram_saving": "30-50% for long contexts with causal eviction",
        },
        "phase1_conclusion": "CausalKV at HF level not viable (overhead > savings). Must integrate at vLLM PagedAttention level.",
        "next_step": "Deploy on Linux server with vLLM >= 0.4.0, implement ThermoBlockManager",
    }

    # === Save ===
    report_path = REPORT_DIR / "v22_five_todos_report.json"
...
def print_report(r):
    ...
    for model_key in ["minicpm5_1b_bf16_20prompts", "qwen25_7b_bf16_10prompts"]:
    ...
    for affine_key in ["minicpm5_1b_affine_compression", "qwen25_7b_affine_compression"]:
        if affine_key not in r:
            continue
        a = r[affine_key]
        print(f"\n--- [待办5] 仿射子空间压缩 ({affine_key.replace('_', ' ')}) ---")
        print(f"  Ridge dim: {a.get('ridge_dim')}, Compression: {a.get('compression_ratio', 0):.3f}")
        print(f"  Explained var: {a.get('explained_variance', 0):.4f}")
        print(f"  Cosine sim: {a.get('cosine_sim_mean', 0):.4f}, MSE: {a.get('mse_mean', 0):.6f}")
        print(f"  P_c/P_raw (monitor): {a.get('pc_ratio_monitor_mean', 0):.4f}")
        print(f"  Per-token P_c/P_raw: {a.get('per_token_pc_ratio', 0):.4f}")

    if "vllm_integration_design" in r:
        v = r["vllm_integration_design"]
        print(f"\n--- [待办4] vLLM集成设计 ---")
        print(f"  状态: {v['status']}")
        print(f"  阻塞: {v['current_blocker']}")
        print(f"  集成点: {v['proposed_interface']['integration_point']}")
        print(f"  Phase 1结论: {v['phase1_conclusion']}")
