...
REPORT_PATH = Path(__file__).parent / "v20_ornith_all_layers_report.json"

PROMPT = "Explain the concept of entropy in thermodynamics in two sentences."

def main():
    print("=" * 60)
    print("Ornith-1.0-9B All-Layer Thermodynamic Scan")
    print("=" * 60)
    ...
    print("\n[2/4] Setting up hooks on ALL 32 layers...")
    ...
    for li in range(num_layers):
        ...
        if len(states) < 3:
            print(f"  {li:5d} | {lt:>17} | SKIPPED (only {len(states)} states)")
            continue
