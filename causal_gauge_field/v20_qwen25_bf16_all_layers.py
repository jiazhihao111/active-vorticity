...
REPORT_PATH = Path(__file__).parent / "v20_qwen25_bf16_all_layers_report.json"

PROMPT = "Explain the concept of entropy in thermodynamics in two sentences."

def main():
    print("=" * 60)
    print("Qwen2.5-7B BF16 All-Layer Thermodynamic Scan")
    print("=" * 60)

    print("\n[1/4] Loading model (bf16)...")
    ...
    results = {}
    print(f"\n  {'Layer':>5} | {'Pc/Praw':>8} | {'cos(Pc)':>8} | {'r(0.95)':>7} | {'|v|':>9} | {'|a|':>9}")
    print("  " + "-" * 65)
    ...
    for li in range(num_layers):
        ...
        print(f"  {li:5d} | {ratio:8.4f} | {cos_mean:8.4f} | {r_eff:7d} | {vel_norms.mean().item():9.2e} | {acc_norms.mean().item():9.2e}")
    ...
    print("QWEN2.5 BF16 ALL-LAYER SCAN COMPLETE")
